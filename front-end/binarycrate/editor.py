...
class EditorView(BCChrome):
    ...
    def get_default_module_form_classes(self):
        ...
        return [getattr(imported_module, name) for name in dir(imported_module) if
                inspect.isclass(getattr(imported_module, name)) and issubclass(getattr(imported_module, name),
                                                                               StudentForm)]
    ...
    def query_project(self):
        ...
        if len(project) == 0:
            ...
            ajaxget('/api/projects/' + self.get_root().url_kwargs['project_id'] + '/',
                    self.projects_api_ajax_result_handler)
    ...
    def get_selected_de_form_controls(self):
        ...
        if self.selected_de and not self.program_is_running:
            ...
            if self.selected_item != '':
                ...
                svg_list.extend([
                    svg('rect', {'x': selected_form_item['x'],
                                 ...
                                 'onmouseup': self.on_mouse_up,
                                 'oncontextmenu': lambda e, form_item_id=selected_form_item[
                                     'id']: self.contextmenu_control(form_item_id, e)}),
                    ...
                    svg('rect',
                        {'id': 'handle-top-right', 'x': selected_form_item['x'] + selected_form_item['width'] - 5,
                         'y': selected_form_item['y'] - 5,
                         'width': 10,
                         'height': 10,
                         'style': "fill:rgb(255,0,0);stroke-width:5;stroke:rgb(255,0,0)",
                    ...
                    svg('rect',
                        {'id': 'handle-bottom-right', 'x': selected_form_item['x'] + selected_form_item['width'] - 5,
            ...
            ret.append(svg('svg', {'id': 'preview-svg', 'height': '100%', 'width': '100%',
                                   'oncontextmenu': self.contextmenu_preview, 'z-index': -5,
                                   'onmousedown': self.clear_selected_item, 'onmouseup': self.on_mouse_up}, svg_list))
    ...
    def changeProperty_ok(self, e, form_values):
        ...
        if get_form_item_property(fi['type'])[self.current_prop_name] == FormItemPropType.INT:
            ...
            value = 'none' if form_values['chkEmpty'] else 'rgb({},{},{})'.format(form_values['txtRed'],
                                                                                  form_values['txtGreen'],
                                                                                  form_values['txtBlue'])
            ...
            Modal("newFile", "New File", [
                form([
                    div({'class': 'form-group'}, [
                        ...
                        html_input(
                            {'type': "text", 'class': "form-control", 'id': "txtFileName", 'placeholder': "New File"}),
                    ]),
                ]),
            ], self.newFile_ok),
            Modal("newFolder", "New Folder", [
                form([
                    div({'class': 'form-group'}, [
                        ...
                        html_input({'type': "text", 'class': "form-control", 'id': "txtFolderName",
            ...
            Modal("changeProperty", "Change Property " + self.current_prop_name if self.current_prop_name else '', [
                form([
                    div({'class': 'form-group'}, [
                        ...
                        html_input(
                            {'type': "text", 'class': "form-control", 'id': "txtValue", 'placeholder': "New Value",
                             'value': get_current_form_item_prop_val()}),
                    ]),
                ]),
            ], self.changeProperty_ok),
            Modal("changePropertyBoolean",
                  "Change Boolean Property " + self.current_prop_name if self.current_prop_name else '', [
                      form([
                          div({'class': 'form-group'}, [
